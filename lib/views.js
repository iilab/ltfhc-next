		...
		if (doc.collection == 'visits' && doc.class == 'entry') {
			...
			for (report in reports) { // for each document loop over all reports available 
				...
				var matches = {
                    ...
                    report_screening_1: ["disease_diagnosis"],
                    report_screening_2: ["disease_diagnosis"],
                    report_screening_3: ["disease_diagnosis"],
					report_dtc: ["diarrhea"],
...
//				if (true) {
				if (DEBUG) emit(["log before optimisation: doc.reasons", doc.reasons, "log report", report,"matches report", matches[report], doc.reasons.filter(function (n) { return matches[report].indexOf(n) != -1}).length])
				...
				//if (~doc.reasons.join(" ").indexOf(report.split("report_")[1].concat("_"))) {
					for (indicator in reports[report].properties) {
						...
						if (DEBUG) emit(["log line condition: doc.reasons", doc.reasons, "matches report", matches[report], "log line condition", cdtn])
						if (cdtn && reports[report].properties[indicator].line_number) {
							try {
								with(doc){
									...
									if (DEBUG) emit(["log line result: log line condition", cdtn, "result", result])
									if (result === null) { 
										emit([ "error", "", "", "", "", "", "", report, "line_condition_result", indicator, "condition result null" + cdtn ],{})																
									} else if (typeof result === "undefined" || result === false) {
                                        emit([doc.date_created.split("-")[0], doc.date_created.split("-")[1], report,  parseInt(line_num[1]), line_num[2], indicator, reports[report].properties[indicator].en, "all", result ], { bucket: "all"})    
                                    } else {
										if (result) {
															...
															if (typeof result === "undefined" || result === null) { 
																...
																if (result) { // column cdtn evaluation is truthy
                                                                    ...
                                                                    if (reports[report].properties[indicator].line == "singlecell") {
                                                                        emit([doc.date_created.split("-")[0], doc.date_created.split("-")[1], report,  parseInt(line_num[1]), line_num[2], indicator, reports[report].properties[indicator].en, "singlecell", result ], { bucket: "singlecell"})    
                                                                    }
                                                                    else {
