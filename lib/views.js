...
exports.reports_entries_static = {
	map: function(doc) {/*
		var bucket = "";
		...
		if (doc.collection == 'visits' && doc.class == 'entry') {
			...
			for (report in reports) {
				for (indicator in reports[report].properties) {
					...
					column_loop: // label used for breaking in nested loops
							...
							try {
								with(doc){
									...
									if (typeof result === "undefined" || result === null) { 
										emit(["error", report, col, "column condition result null or undefined: " + cdtn ],{})																
									} else {
										if (result) {
											emit(["INFO", col])
											bucket = col.toString;
								...
								if (e instanceof SyntaxError) {
	    								emit(["error", report, indicator, "syntax error in report in condition: " + cdtn],{})
									} else
									{
	    								emit(["error", report, indicator, "error in condition: " + cdtn ],{})	
									}
							}
						} else {
							emit(["error", report ,col, "column missing a condition field"],{})
						}
					}
					...
					if (cdtn && reports[report].properties[indicator].line_number) {
						try {
							with(doc){
								var result = eval('(' + cdtn + ')')
								if (typeof result === "undefined" || result === null) { 
									emit(["error", report, indicator, "condition result null or undefined: " + cdtn ],{})																
								} else {
									if (result) {
										var line_num = reports[report].properties[indicator].line_number.split(/(\d*)/)
										emit([report, parseInt(line_num[1]), line_num[2], reports[report].properties[indicator].en, bucket, result ], { bucket: bucket})										
									}
								}
							}
						} catch (e) {
						...
						if (reports[report].properties[indicator].line_number != null && reports[report].properties[indicator].en != null ) {
							var line_num = reports[report].properties[indicator].line_number.split(/(\d*)/)
							emit([report, parseInt(line_num[1]), line_num[2], reports[report].properties[indicator].en, "header", "" ], { bucket: "header"})	
						} else {
							emit(["error", report ,indicator, "line number or language label not defined"],{})						
						}			
					} else
					{
						emit(["error", report ,indicator, "unknown error"],{})						
				    }
				}
			}
		}
