...
exports.reports_entries = {
			...
			for (report in reports) {
				for (indicator in reports[report].properties) {
					...
					if (cdtn && reports[report].properties[indicator].line_number) {
						try {
							with(doc){
								...
								if (typeof result === "undefined" || result === null) { 
									...
									if (result) {
										emit([report, reports[report].properties[indicator].line_number, indicator, bucket, result ], {report: report, line_number: reports[report].properties[indicator].line_number, indicator: reports[report].properties[indicator].en, bucket: bucket})										
									}
								}
							}
						} catch (e) {
						...
						//break // this should be a header so it's fine.
	...
	reduce: function(keys, values, rereduce) {
		if (rereduce) { //attempt at writing rereduce
			/* var result = {
				report: keys[0][0][0],
				line_number: values[0].line_number,
				indicator: values[0].indicator,
				total: function () { for (val in values) { total++ } return total } 
			}*/
			return {error: "rereduce!"};
		} else {
			var result = {
				report: keys[0][0][0],
				line_number: values[0].line_number,
