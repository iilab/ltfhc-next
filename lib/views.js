...
exports.report_entries = {
	map: function(doc) {
		var bucket = "",
			indicator = "";

//		var db = require('db').current().getDoc("reports/schema/anc", null, function (err, response) {
//			var reports_anc = JSON.parse(response);
//		})

		if (doc.collection == 'visits' && doc.class == 'entry') {
			var bucket = (doc.patient.approxage < 20) ? "below20" : "above20";

//			emit([reports_anc.properties.gestational_age_below_12_weeks.en])
			emit(["report_anc", (doc.anc_first_visit.gestational_age < 12) ? "gestational_age_below_12_weeks" : "gestational_at_week_12_or_more", bucket])
			emit(["report_anc", (doc.anc_first_visit.gestational_age) ? "total_attendance_of_first_visits(2a+2b)" : null, bucket])

			emit(["report_anc", (doc.anc_first_visit.pregnancy_test) ? "number_of_pregnant_women_tested_first_visit" : null, bucket])
