				...
				for (indicator in reports[report].properties) {
					...
					if (cdtn && reports[report].properties[indicator].line_number) {
						try {
											...
											if (cdtn) {
												try {
													with(doc){
														var result = eval('(' + cdtn + ')')
														if (typeof result === "undefined" || result === null) { 
															emit(["error", report, "column_condition" , col, "column condition result null or undefined: " + cdtn ],{})																
														} else {
															if (result) { // column cdtn evaluation is truthy
																//emit(["error", report, "info", col, "bucket assignment:" + cdtn])
																bucket = col;
																emit([report, parseInt(line_num[1]), line_num[2], indicator, reports[report].properties[indicator].en, bucket, result ], { bucket: bucket})										
															}
														}
													}
												} catch (e) {
													if (e instanceof SyntaxError) {
						    								emit(["error", report, "column_condition", indicator, "syntax error in report in condition: " + cdtn],{})
														} else
														{
						    								emit(["error", report, "column_condition", indicator, "error in condition: " + cdtn ],{})	
														}
												}
											} else {
												emit(["error", report, "column_condition", col, "column missing a condition field"],{})
											}
										}
									}
								}
							...
							if (e instanceof SyntaxError) {
    								emit(["error", report, "line_condition", indicator, "syntax error in report in condition: " + cdtn],{})
								} else
								{
    								emit(["error", report, "line_condition", indicator, "error in condition: " + cdtn ],{})	
								}
						}
						...
						if (reports[report].properties[indicator].line_number != null && reports[report].properties[indicator].en != null ) {
							// header line
							var line_num = reports[report].properties[indicator].line_number.split(/(\d*)/)
							emit([report, parseInt(line_num[1]), line_num[2], indicator, reports[report].properties[indicator].en, "header", "" ], { bucket: "header"})	
						} else {
