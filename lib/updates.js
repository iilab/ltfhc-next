    ...
    _.each(submitted_dates, function (value, key){
        if (value.date) {
            ...
            _.chain(schema_date_periods[key])
             .map(function (value, key) { return {value: value, key: key}})
             .each(function (item) {
                 doc = ksutils.updateCurrent(doc, date, item.value.split("_")[1], item.key)
             })
            ...
            doc = ksutils.updateDate(doc, current, value.current.split("_")[1], value.current.split("_")[2])
...
exports.report = function(doc, req) {
    ...
    return [doc, dutils.redirect(req, '/reports')] //'/' + doc._id)];
