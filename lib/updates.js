...
exports.patient = function(doc, req) {
    ...
    var current_overwrites = Array() 

    current_overwrites = _.object(_.map(req.form, function(k, v) { // copy to doc submitted value from form at same key.
        if (k.indexOf("current_") != 1 && doc[k] != v) {
            return ['date_' + k.split("_")[1], true]
        }
    }))
    ...
    for (var k in req.form) { // copy to doc submitted value from form at same key.
        if (k != "_submit" && req.form[k] !== "") { // except special hidden field _submit holding clicked submit button value.
            if (!current_overwrites[k]) doc[k] = req.form[k]
        }
    }
    ...
    _.each(doc, function (e, k) {
        if (k.indexOf('current_') === 0) {
            doc = ksutils.updateDate(doc, k.split("_")[1], k.split("_")[2], current_overwrites[k])
        }
    })
