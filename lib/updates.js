...
exports.patient = function(doc, req) {
    events.once('afterResponse', function() {
        ...
        var schemaSource =  prefixSource + "/schema/" + ((req.cookie[" userLang"]) || "en") + "/patient.json"
        var optionsSource = prefixSource + "/options/" + ((req.cookie[" userLang"]) || "en") + "/patient.json"
    ...
    submitted_dates['created'] = { date: "created"};

    // get date fields from the patient schema that relate to submitted date fields
    var prefixSource =  "schema/forms/" + settings.country
    var schemaSource =  prefixSource + "/schema/" + ((req.cookie[" userLang"]) || "en") + "/patient"
            
    schema_date_fields = _.chain(schema.load(schemaSource).properties)
                          .map(function (value, key) { return {value: value, key: key}})
    ...
    _.each(submitted_dates, function (value, key){
        if (value.date) {
            ...
            if (key == "created") date = doc.date_created
