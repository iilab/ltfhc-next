    ...
    db.getView(ddoc, view, q, function(err, data) {
        ...
        for (var row in data.rows) {
            ...
            if (data.rows[row].id.split("_")[1] != "report") {
                ...
                ksutils.createReport("report_" + data.rows[row].id.split("_")[2], moment(data.rows[row].value.date_scheduled).subtract(1, 'months'), function(err, res){
                    if (err) {
                        console.log(err)
                        console.log("Error with scheduled trigger")
                        console.log("With report")
                        console.log("report_" + data.rows[row].id.split("_")[2])
                    } else {
                      //location.href = dutils.getBaseURL();
                    }
                })
