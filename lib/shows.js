...
exports.reports_entries = function (doc, req) {
    events.once('afterResponse', function () {

        utils.updateTopNav('reports_entries');
...
//            renderReportsControls(req);
			...
			db.getView(ddoc, view, q, function (err, data) {
				...
				for (var i in data.rows) {
		            ...
		            docs[i].line_number = data.rows[i].key[1];
		            docs[i].indicator = data.rows[i].key[2];
                ...
                $('[data-page=reports_entries] #reports_entries').spreadsheet({
                    columns: [
	                    {
                        ...
                        {
                            label: $.kansotranslate('Below 20'),
                            property: ['below20'],
                            type: 'string'
                        },
                        {
                            label: $.kansotranslate('Above 20'),
                            property: ['above20'],
                            type: 'string'
                        },
                        {
                            label: $.kansotranslate('Total'),
                            property: ['total'],
                            type: 'string'
                        }
                    ],
    ...
    return {
        info: info,
        title: info.translate('Reports'),
        content: templates.render('reports_entries.html', req, props)
    };
};
