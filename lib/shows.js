				...
				var datatable_options = {
					...
					"aoColumns": [{
						...
						"mRender": function(data, type, full) {
							...
							return '<a href="' + baseURL + '/patient/' + full._id + '"><i class="icon icon-2x icon-edit"></a>';
						}
					}]
				}
...
exports.patient = function(doc, req) {
	events.once('afterResponse', function() {
		...
		setupContext(req, function(err) {
			...
			$("#patient #patient-form #form").alpaca(alpaca_options);
		})
	})
...
exports.patient_new = function(doc, req) {
	events.once('afterResponse', function() {
			...
			var alpaca_options = {
				...
				"postRender": function(control) {
					...
					$("#patient-action[value='save']").click(function(_ev) {
						...
						db.saveDoc(data, function(errsave, datasave) {
							if (errsave) return alert('Save failed. ' + errsave);
							if (!datasave.ok) return alert('Save failed.');
							console.log(datasave)
			        $modal = $(templates.render('modal_info.html', {}, {
			            title: "Patient Data Saved"
			        }));

			        $modal.modal('show');
			        
							return [null, dutils.redirect('patient?_id=' + req.query["_id"])];
		...
		setupContext(req, function(err) {
			...
			db.getDoc(req.query["_id"], q, function(err, data) {
				...
				var alpaca_options = {
					...
					"postRender": function(control) {
						var cancelButton = $("<button id='patient-action' class='btn btn-lg btn-default' type='submit' value='cancel' onclick='history.go(-1);'>Cancel</button>");
						var submitButton = $("<button id='patient-action' class='btn btn-lg btn-primary pull-right' type='submit' value='save'>Save</button>");
						control.getEl().append(cancelButton);
						control.getEl().append(submitButton);

						$("#visit-action[value='save']").click(function(_ev) {
							var docdata = control.getValue();
							Object.keys(docdata).map(function(key){
								data[key] = docdata[key];
							})
						...
						$('#visit-form').submit(function(_ev) {
							_ev.stopPropagation();
							_ev.preventDefault();
						});
						
						$('#loader').hide();
						$("#visit #visit-form").show();
				...
				$("#visit #visit-form").alpaca(alpaca_options).hide();
				...
				var datatable_options = {
					...
					"aoColumns": [{
						...
						"mRender": function(data, type, full) {
							// 'full' is the row's data object, and 'data' is this column's data
							// e.g. 'full[0]' is the comic id, and 'data' is the comic title
							return '<a href="' + baseURL + '/visit?_id=' + full._id + '"><i class="icon icon-2x icon-edit"></a>';
						}
					}]
				}
				datatable_options.aaData = docs
	...
	events.once('afterResponse', function() {
		...
		setupContext(req, function(err) {
			...
			db.getView(ddoc, view, q, function(err, data) {
				...
				$('[data-page=report_calculated] #reports_calculated').spreadsheet({
					columns: columns,
					data: docs
				});
			});
