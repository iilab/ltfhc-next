...
var runSchedule = function () {
    ...
    db.getView(ddoc, view, q, function(err, data) {
        ...
        // schedule docs relating to reports
        var report_rows = _.filter(data.rows, function (row) {
            return row.id.split('_')[1] === 'report';
        });

        // add report_id property to rows
        report_rows = _.map(report_rows, function (row) {
            row.report_id = 'report_' + row.id.split('_').slice(2).join('_');
            return row;
        });

        // create object keyed by report id with super-reports combined
        var scheduled_reports = _.reduce(report_rows, function (acc, row) {
            var id = reports[row.report_id].parent || row.report_id;
            if (!acc[id]) {
                acc[id] = [];
            }
            acc[id].push(row);
        ...
        for (var report_id in scheduled_reports) {
            // report scheduled update.
            var rows = scheduled_reports[report_id];
            var dt = moment(rows[0].value.date_scheduled).subtract(1, 'months');
            var ids = _.pluck(rows, 'report_id');
            ksutils.createReport(ids, dt, function(err, res){
                if (err) {
                    console.log(err)
...
exports.reports = function(doc, req) {
    events.once('afterResponse', function() {
        ...
        setupContext(req, function(err) {
            ...
            baseURL = require('duality/core').getBaseURL(req);
            // check report scheduling is in place
            var reports_schema = require("views/lib/reports_schema")
            var reports = reports_schema.reports()
            for (var report in reports) {
                db.getDoc("schedule_" + report, function(e,schedule_doc) {
            ...
            populateReportsTable(req, view, q);
            ...
            $('input.daterange').each(function () {
                ...
                function updateValue(start, end) {
                    ...
                    $('[name="daterange-start"]', el.parent()).val(
                        start.toISOString()
                    );
                    $('[name="daterange-end"]', el.parent()).val(
                        end.toISOString()
                    );
                }
                el.daterangepicker({
                    ranges: {
                        ...
                        'Last Month': [
                            window.moment().subtract('month', 1).startOf('month'),
                            window.moment().subtract('month', 1).endOf('month')
                        ]
                    },
                ...
                if (!el.val()) {
                    var start = window.moment().startOf('month');
                    var end = window.moment().endOf('month');
            ...
            $('.btn-preview').click(function (ev) {
                ...
                var report = $('[name=report]', form).val();
                var start = $('[name=daterange-start]', form).val();
                var end = $('[name=daterange-end]', form).val();
                window.location = (
                    './report_calculated?' + querystring.stringify({
                        report: report,
                        start: start,
                        end: end
                    })
...
var populateReportsTable = function (req, view, q) {
    var db = require('db').use(ksutils.getDBURL());
    ...
    db.getView(ddoc, view, q, function(err, data) {
        ...
        var datatable_options = {
            "oLanguage": {
                "sLengthMenu": "_MENU_ " + $.kansotranslate('reports per page'),
                "sInfo": $.kansotranslate('Showing') + " _START_ " + $.kansotranslate('showing.to') + " _END_ " + $.kansotranslate('showing.of') + " _TOTAL_ " + $.kansotranslate('showing.entries'),
                "sInfoEmpty": $.kansotranslate('Showing') + " 0 " + $.kansotranslate('showing.to') + " 0 " + $.kansotranslate('showing.of') + " 0 " + $.kansotranslate('showing.entries'),
                "sEmptyTable": $.kansotranslate('emptytable'),
                "oPaginate" : {
            ...
            "aoColumns": [{
                ...
                "sTitle": $.kansotranslate("Report Type"),
                "mData": "title"
            }, {
                ...
                "mRender": function(data, type, full) {
                    // 'full' is the row's data object, and 'data' is this column's data
                    // e.g. 'full[0]' is the comic id, and 'data' is the comic title
                    return '<a class="btn btn-primary" href="' + baseURL + '/report/' + full._id + '"><i class="icon icon-list"> ' + $.kansotranslate("Report") + '</a>';
                }
            }]
        }
        ...
        $('#loader').hide();
        $('#reports-table').show();
        $('#search-filter-input').val('Unapproved').change()
    });
};
...
var renderReportsControls = function(req) {
    ...
    $('#search-filter-input').change(function() {
        oTable.fnFilter($(this).val(), 3, false, true, true, false);
    })
}
    ...
    events.once('afterResponse', function() {
        ...
        setupContext(req, function(err) {
            ...
            _.each(docs, function (doc) {
                ...
                // custom table header prefix?
                var thead_prefix = false;
                if (reports[doc.report].overrides) {
                    thead_prefix = reports[doc.report].overrides.thead_prefix;
                }

                $('[data-page=report] #report #report-form #form').append(
                    $('<div/>').spreadsheet({
                        thead_prefix: thead_prefix,
                        columns: columns,
                        data: rows
                    })
                );
            ...
            $('#report #report-name').text(doc.title + " - " + doc.period + " (" + doc.status + ")")        
            $('#report #report-form input[name=title]').val(doc.title)
            $('#report #report-form input[name=period]').val(doc.period)
        ...
        setupContext(req, function(err) {
            ...
            async.forEachSeries(report_ids, function (report_id, cb) {
                ...
                db.getView(ddoc, view, q, function(err, data) {
                    ...
                    // custom table header prefix?
                    var thead_prefix = false;
                    if (reports[report_id].overrides) {
                        thead_prefix = reports[report_id].overrides.thead_prefix;
                    }

                    $('[data-page=report_calculated] #report_calculated #report-form #form').append(
                        $('<div/>').spreadsheet({
                            thead_prefix: thead_prefix,
                            columns: columns,
                            data: rows
