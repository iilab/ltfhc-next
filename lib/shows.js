...
var runSchedule = function () {
    ...
    db.getView(ddoc, view, q, function(err, data) {
        ...
        for (var row in data.rows) {
            ...
            if (data.rows[row].id.split("_")[1] != "report") {
                db.getDoc(data.rows[row].doc.doc_scheduled, function(e, doc) {
                    if (e) { 
                ...
                var report_id = 'report_' + data.rows[row].id.split('_').slice(2).join('_');
                ksutils.createReport(report_id, moment(data.rows[row].value.date_scheduled).subtract(1, 'months'), function(err, res){
                    if (err) {
                        console.log(err)
                        console.log("Error with scheduled trigger")
        ...
        // TODO must be a cleaner way to redirect. Also this seems like callback inception (the movie)
        //var = req, {
        //    isAdmin: isAdmin,
        //    userCtx: userCtx
        //});
        
        //shows.patients(null, req)
        //
    })
}
...
exports.report = function(doc, req) {
    events.once('afterResponse', function() {
        ...
        setupContext(req, function(err) {
            ...
            $('[data-page=report] #report #report-form #form').spreadsheet({
