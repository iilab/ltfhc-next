...
exports.patient = function(doc, req) {
    events.once('afterResponse', function() {
        ...
        setupContext(req, function(err) {
            ...
            var prefixSource =  "schema/forms/" + settings.country
            var schemaSource =  prefixSource + "/schema/" + ((userLang) || "en") + "/patient.json"
            var optionsSource = prefixSource + "/options/" + ((userLang) || "en") + "/patient.json"
...
exports.visit = function(doc, req) {
    events.once('afterResponse', function() {
        ...
        setupContext(req, function(err) {
            ...
            var prefixSource =  "schema/forms/" + settings.country
            var schemaSource =  prefixSource + "/schema/" + ((userLang) || "en") + "/visit.json"
            var optionsSource = prefixSource + "/options/" + ((userLang) || "en") + "/visit.json"
            ...
            db.getView(ddoc, view, q, function(err, data) {
                ...
                var datatable_options = {
                    ...
                    "aoColumns": [{
                        ...
                        "sTitle": $.kansotranslate("Gender"),
                        "sClass": "center",
                        "mData": "patient.gender"
                    }, {
                        "sTitle": $.kansotranslate("Phone"),
                        "sClass": "center",
                        "mData": "patient.phone"
                    }, {
                        "sTitle": $.kansotranslate("Reason for Visit"),
                        "sClass": "center",
                        "mData": "reasons",
                        "mRender": function(data, type, full) {
                            // 'full' is the row's data object, and 'data' is this column's data
...
exports.reports = function(doc, req) {
    events.once('afterResponse', function() {
        ...
        function renderTable(item, table, report, report_periods, docs) {
            ...
            $('.btn-generate-report', table).click(function (ev) {
                ...
                ksutils.generateReport(report, start, end, function (err, res) {
                    ...
                    window.location = baseURL + '/report/' + res.id;
                });
                return false;
            });
        ...
        function getFirstVisit(callback) {
            ...
            db.getView(ddoc, 'visits', q, function (err, data) {
                if (err) {
                    return callback(err);
                }
                var first_visit;
                if (data.rows.length) {
                    first_visit = data.rows[0].value.date_created;
                }
...
exports.report = function(doc, req) {
    events.once('afterResponse', function() {
        ...
        setupContext(req, function(err) {
            ...
            _.each(docs, function (doc) {
                var data = {}, rows = new Array()
                    ...
                    .reduce(function(memo, item) {
                        ...
                        n.label = $.kansotranslate(item.key.split("|")[3])
                ...
                $('[data-page=report] #report #report-form #form')
                    .append($('<h3/>').text(reports[doc.report][userLang] || reports[doc.report].title))
            ...
            var period = moment(doc.period.start).format('YYYY.MM');
            $('#report #report-name').text(
                $.kansotranslate(doc.title) + " - " + period + " (" + doc.status + ")"
            );
...
exports.report_calculated = function(doc, req) {
    ...
    report_ids = _.sortBy(report_ids, function(i) {
        return parseInt(reports[i].title.split('.')[0])
    })
    ...
    events.once('afterResponse', function() {
        ...
        setupContext(req, function(err) {
            ...
            async.forEachSeries(report_ids, function (report_id, cb) {
                ...
                // deal with manual reports 

                if (reports[report_id].manual) {
                    
                    $('#loader').hide();
                    var report_properties = reports[report_id].properties
                    var rows = new Array();
                    var predefined = new Array();
                    for (var i in report_properties) {
                        rows.push({
                            line_number: report_properties[i].line_number,
                            indicator: i,
                            label: report_properties[i][userLang] || report_properties[i].en
                        })
                    ...
                    for (var col in reports[report_id].definitions.columns) {
                        ...
                        new_col.label = reports[report_id].definitions.columns[col][userLang] || reports[report_id].definitions.columns[col].title || col;
                    ...
                    $('[data-page=report_calculated] #report_calculated #report-form #form')
                        .append($('<h3/>').text(reports[report_id][userLang] || $.kansotranslate(reports[report_id].title)))
                        .append($('<div/>').spreadsheet({
                            overrides: { predefined: predefined},
                            columns: columns,
                            data: rows
                        }));
                    ...
                    db.getView(ddoc, view, q, function(err, data) {
                        ...
                        $('[data-page=report_calculated] #report_calculated #report-form #form')
                            .append($('<h3/>').text(reports[report_id][userLang] || reports[report_id].title))
                            .append($('<div/>').spreadsheet({
    ...
    var title = (
        info.translate(reports[report_ids[0]].parent_title) ||
        _.map(report_ids, function (id) { return info.translate(reports[id].title); }).join(', ')
    );
...
exports.users = function(_doc, _req) {
    ...
    var map_user_type = function(_type) {
        ...
        var map = {
            'clinic-manager': ['kujua_user', 'data_entry', 'clinic_admin'],
...
var setupContext = function(req, callback) {
    isUser = utils.isUser(req.userCtx);
    isClinicAdmin = utils.isClinicAdmin(req.userCtx);
    isDistrictAdmin = utils.isDistrictAdmin(req.userCtx);
    userLang = req.cookie.userLang || req.cookie['userLang'] || req.cookie[' userLang'] || "en"
    userLang = (userLang != "undefined") ? userLang : "en"

    console.log(['setupContext', userLang, req]);

    if (isUser || isClinicAdmin || isDistrictAdmin) {
        callback();
    } else {
        //		callback();
