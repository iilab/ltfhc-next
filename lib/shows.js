...
var renderPatientsControls = function(req) {
    ...
    $('.page-header .controls').first().html(
        templates.render('patients_controls.html', req, {query:q})
    ).show();
    ...
    events.once('afterResponse', function () {
        ...
        setupContext(req, function(err) {
            ...
            var db = require('db').current(),
                ...
                lockRows = true,
                q = {
					startkey: [req.query["lastname"], req.query["firstname"]],
					endkey: [req.query["lastname"], req.query["firstname"], {}],
                    include_docs: true
                };
			...
			db.getView(ddoc, view, q, function (err, data) {
				...
				$("#patients #patient-form").alpaca({
					"schemaSource": "../prototype/schema/patient.json",
					"optionsSource": "../prototype/options/patient.json",
					"data": docs[0],
					"postRender": "",
					"ui": "bootstrap",
					"type": "create"
	...
	events.once('afterResponse', function() {
		...
		setupContext(req, function(err) {
			...
			var db = require('db').current(),
				view = 'visits',
				q = {
					startkey: ["2011", "01", "27", "Doe", "John"],
					endkey: ["2011", "01", "27", "Doe", "John", {}]
				};
			...
			db.getView(ddoc, view, q, function(err, data) {
				...
				$("#visits").alpaca({
//					"schemaSource": "../prototype/schema/anc_first_visit.json",
//					"optionsSource": "../prototype/options/anc_first_visit.json",
//					This should be the visit container which has all of the subforms as dependencies.
					"schemaSource": "../prototype/options/visit.json",
					"optionsSource": "../prototype/options/visit.json",
					"dataSource": "",
					"postRender": "",
					"ui": "bootstrap",
					"type": "create"
	...
	return {
		...
		content: templates.render('visits.html', req, {
			nav: 'visits'
		})
	};
}
...
exports.visits_connector = function(doc, req) {
	events.once('afterResponse', function() {
			...
			db.getView(ddoc, view, q, function(err, data) {
				...
				var connector = Alpaca.Connector.extend({
				    ...
				    loadSchema: function(resource, successCallback, errorCallback)
				    {
				        ...
				        {
				            resource = "../prototype/schema/" + resource;
				    ...
				    loadOptions: function(resource, successCallback, errorCallback)
				    {
				        if (typeof(resource) == "string")
				        {
				            resource = "../prototype/options/" + resource;
				            if (resource.indexOf(".json") == -1) {
				                resource += ".json"
				            }
				        }
				    ...
				    loadReferenceSchema: function(resource, successCallback, errorCallback)
				    {
				        if (typeof(resource) == "string")
				        {
				            resource = this.adjustPath(resource, "../prototype/schema", "../prototype/sub/schema");
				            if (resource.indexOf(".json") == -1) {
				                resource += ".json"
				            }
				        }
				    ...
				    loadReferenceOptions: function(resource, successCallback, errorCallback)
				    {
				        if (typeof(resource) == "string")
				        {
				            resource = this.adjustPath(resource, "../prototype/options", "../prototype/sub/options");
				            if (resource.indexOf(".json") == -1) {
				                resource += ".json"
				            }
				        }
        ...
        setupContext(req, function(err) {
            ...
            var db = require('db').current(),
                ...
                q = {
                    startkey: ['district_hospital'],
                    endkey: ['district_hospital', {}],
                    include_docs: true //Use this to get the reports entries linked.
                };
			...
			db.getView(ddoc, view, q, function (err, data) {
                ...
                $('[data-page=reports] #reports').spreadsheet({
                    columns: [
                        {
                            label: $.kansotranslate('First Name'),
                            property: ['firstname'],
                            type: 'string'
                        },
                        {
                            label: $.kansotranslate('Last Name'),
                            property: ['lastname'],
                            type: 'string'
                        },
                        {
                            label: $.kansotranslate('Clinic ID'),
                            property: ['clinicid'],
                            type: 'string'
                        },
                        {
                            label: $.kansotranslate('Govt ID'),
                            property: ['governmentid'],
                            type: 'string'
                        },
                        {
                            label: $.kansotranslate('HIV ID'),
                            property: ['hivid'],
                            type: 'string'
                        },
                        {
                            label: $.kansotranslate('Phone'),
                            property: ['phone'],
                            type: 'string',
                            validation: 'phone'
                        },
                        {
                            label: $.kansotranslate('Age'),
                            property: ['approxage'],
                            type: 'string'
                        },
                        {
                            label: $.kansotranslate('Village'),
                            property: ['village'],
                            type: 'string'
                        },
                        {
...
exports.reports_calculated = function (doc, req) {
    events.once('afterResponse', function () {

        utils.updateTopNav('reports_calculated');
        ...
        setupContext(req, function(err) {
            ...
            var db = require('db').current(),
                ...
                q = {
					startkey: [req.query["report"]],
					endkey: [req.query["report"], {}],
                    group_level: 4,
//					reduce:true,
//					rereduce:false
                };
...
//            renderReportsControls(req);
			...
			db.getView(ddoc, view, q, function (err, data) {
				...
				var docs = {};
				for (var i in data.rows) {
				    docs[i] = data.rows[i].value;
		            docs[i].line_number = data.rows[i].key[1] + data.rows[i].key[2];
		            docs[i].indicator = data.rows[i].key[3];
					docs[i].header = data.rows[i].value.header;
				}
				...
				for (var col in reports[req.query["report"]].definitions.columns) {
					var obj = {}
					obj.label = reports[req.query["report"]].definitions.columns[col].title
					obj.property = col
					obj.type = 'string'
					columns.push(obj)
				}

                $('[data-page=reports_calculated] #reports_calculated').spreadsheet({
                    columns: columns,
                    data: docs
                });
            });
    ...
    return {
        info: info,
        title: info.translate('Reports Calculated'),
        content: templates.render('reports_calculated.html', req, props)
    };
};
...
var renderReportsControls = function(req) {
    ...
    $('.page-header .controls').first().html(
        templates.render('reports_controls.html', req, {query:q})
    ).show();
