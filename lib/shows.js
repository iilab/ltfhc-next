        ...
        setupContext(req, function(err) {
            if (err) {
                console.error('setupContext error', err);
						...
						if (true) { // ongoing
							//maybe display count of patients that are counted in the reports.
        ...
        setupContext(req, function(err) {
            ...
            db.getView(ddoc, view, q, function(err, data) {
                ...
                for (var i in data.rows) {
                    ...
                    rows[i].line_number = data.rows[i].key[1] + data.rows[i].key[2];
                    rows[i].indicator = data.rows[i].key[3];
                    rows[i].label = data.rows[i].key[4];
                    if (data.rows[i].value.header) {
                        rows[i].header = data.rows[i].value.header;
                        delete rows[i].total
                    }
                    for (var report_cell in doc) {
                        if (report_cell.indexOf("|") != -1) {
                            report_col = report_cell.split("|")[1]
                            report_row = report_cell.split("|")[0]
                            if (rows[i].indicator == report_row) {
                                rows[i][report_col] = doc[report_cell]
                                console.log("report_cell:" + report_cell + " -> doc[report_cell]: " + doc[report_cell])
                            }
                        }
                    }
                }
    ...
    events.once('afterResponse', function() {
        ...
        setupContext(req, function(err) {
            ...
            var db = require('db').use(ksutils.getDBName()),
                view = 'reports_calculated',
                q = {
                    startkey: ["2014", "02", req.query["report"]],
                    endkey: ["2014", "02", req.query["report"], {}],
                    group_level: 7,
                    //					reduce:true,
                    //					rereduce:false
                };
